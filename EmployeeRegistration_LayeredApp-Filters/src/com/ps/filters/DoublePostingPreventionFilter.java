...
public class DoublePostingPreventionFilter extends HttpFilter {
	...
	public void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain)
		...
		if(method.equalsIgnoreCase("GET")) {
			//generate Session token and make it server side token, also make client side token in the form page
			ses.setAttribute("sToken", new Random().nextInt(1000));
			chain.doFilter(req, res);
		}
		else {
			//read server Token, client Token values
			System.out.println("req param value :: "+req.getParameter("cToken"));
